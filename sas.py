...
def deleteuser():
    ...
    if l.delete(delete_id) == True and delete_id != "":
        Label(screen1, text="Delete Sucessfully", fg="green", font=("calibri", 11)).place(x=250, y=290)
    else:
        Label(screen1, text="ID not found", fg="red", font=("calibri", 11)).pack()
def show_user():
    l.showuser()
...
def details2():
    ...
    Label(screen1, text="").pack()
    label1 = Label(screen1, text="CUSTOMER_ID:", font=("arial", 20, "bold"), fg="black").place(x=0, y=20)
    user_id = StringVar()
    entry_box = Entry(screen1, textvariable=delete_cid, width=25, bg="lightgreen").place(x=250, y=30)
    Button(screen1, text="REMOVE", width="10", height="2", bg="RED",command=deleteuser,font=("Calibri", 13)).place(x=250, y=230)
...
def main_screen():
    ...
    canvas=Canvas(screen,width=150,height=150,)
    image=ImageTk.PhotoImage(Image.open("C:\\gui\\micky.png"))
